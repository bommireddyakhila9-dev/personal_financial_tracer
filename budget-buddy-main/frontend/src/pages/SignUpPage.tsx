...
export default function SignUpPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Firebase signs users in immediately on account creation.
      // Sign out to enforce explicit login as requested.
      await logOut();
      navigate("/sign-in", {
        replace: true,
        state: { signupSuccess: "Account created. Please sign in." },
      });
    } catch (err) {
        ...
        <Card className="grid w-full overflow-hidden border-slate-200/70 bg-white/90 shadow-2xl backdrop-blur md:grid-cols-2">
          <section className="hidden bg-gradient-to-br from-slate-900 via-slate-800 to-cyan-900 p-10 text-white md:block">
            ...
            <p className="mt-4 max-w-sm text-sm text-slate-200">
              Create your account to start tracking expenses, budgets, and savings in one place.
            </p>
          ...
          <CardContent className="p-6 sm:p-10">
            <div className="mx-auto w-full max-w-md space-y-6">
              <div>
                <p className="text-xs font-medium uppercase tracking-[0.18em] text-cyan-700">Get Started</p>
              ...
              <p className="text-center text-sm text-slate-600">
                ...
                <Link to="/sign-in" className="font-medium text-cyan-700 hover:text-cyan-800">
                  Sign in
                </Link>
              </p>
            </div>
